  ...
  try {
    ...
    iyzipay.payment.create(request, (err, result) => {
      ...
      if (result.status === 'success') {
        ...
        return res.status(500).json({ status: 'error', message: 'Ödeme başarız', error: result.errorMessage });
      }
    });
  } catch (e) {
...
app.post('/send-email', async (req, res) => {
  try {
    // req.body üzerinden gelen bilgileri kullanarak sendEmail fonksiyonunu çağırabilirsiniz.
    const emailOptions = {
